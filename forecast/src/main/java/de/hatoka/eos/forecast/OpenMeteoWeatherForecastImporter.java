 ...
 * 
 * Example API call:
 * https://api.open-meteo.com/v1/forecast?latitude=51.0262&longitude=11.5164&hourly=sunshine_duration&forecast_days=3&timezone=UTC
 */
@Singleton
public class OpenMeteoWeatherForecastImporter extends AbstractWeatherForecastImporter
{
     ...
     */
    private String buildApiUrl(MeteoMediaStation station)
    {
        return String.format("%s?latitude=%.4f&longitude=%.4f&hourly=sunshine_duration&forecast_days=%s&timezone=%s",
                API_BASE_URL,
                station.getLatitude(),
    ...
    private String downloadJsonData(URI apiUrl) throws IOException, InterruptedException
    {
                                               ...
                                               .build())
        {
            ...
            {
                throw new IOException("OpenMeteo API request failed with status code: " + response.statusCode() + 
    ...
    private Map<ZonedDateTime, Integer> convertToSunshineDurationMap(OpenMeteoResponse response)
    {
        ...
        {
            logger.warn("Time and sunshine_duration arrays have different sizes: {} vs {}", 
                       times.size(), sunshineDurations.size());
            return result;
        }
        ...
        for (int i = 0; i < times.size(); i++)
        {
            try
            {
                ...
                if (timeString.contains("T") && !timeString.contains("Z") && !timeString.contains("+") && !timeString.substring(timeString.indexOf("T")).contains("-"))
                {
                ...
                // Ensure the value is within valid range (0-60 minutes)
                sunshineMinutes = Math.max(0, Math.min(60, sunshineMinutes));
            ...
            {
                logger.warn("Failed to parse time entry at index {}: '{}', sunshine: {}", 
                           i, times.get(i), sunshineDurations.get(i), e);
            }
        }
